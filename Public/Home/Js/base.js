...
function Base(args) {
    ...
    if (typeof args == 'string') {
        ...
        if (args.indexOf(' ') != -1) {
            ...
            var childElements = []; //存放临时节点的数组,解决被覆盖的问题
            var node = []; //用来存放父节点
        ...
        if (args != undefined) { //这里需要判断 undefined的对象
...
Base.prototype.getName = function (name) {
    var names = docuemnt.getElementsByName(name);
...
Base.prototype.ge = function (num) {
    return element = this.elements[num];
}
...
//设置物体居中
Base.prototype.center = function (width, hieght) {
...
Base.prototype.animate = function (obj) {
    for (var i = 0; i < this.elements.length; i++) {
        ...
        if (alter != undefined && target == undefined) {
            ...
            throw new Error('alert增量或target目标量必须传一个！');
        }
